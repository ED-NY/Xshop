const vm = new Vue({
    ...
    methods: {
        ...
        judge() {
            if (this.username.length < 15 && this.username.length > 1) {
                if (this.userId.length < 3) {
                    this.into.id = this.userId;
                    this.into.userCode = this.username;
                    axios.post('/user/judge', this.into).then(
                        (res) => {
                            if (this.username === res.data[0].userCode && this.userId === res.data[0].id) {
                            } else {
